...
# 创建一个话题。要求user级别的权限。这个项目好像没有修改密码的接口。
...
def create(urlname):
    ...
    if node.role == 'admin' and not g.user.is_admin:
        flash(_('You have no permission in this node.'), 'warn')
        return redirect(url_for('node.view', urlname=urlname))
    # 新建一个form对象， 表单验证通过就可以保存数据。表单验证不通过就展示表单。
    # 对于表单需要ajax验证的时候不能满足要求。
    form = TopicForm()
...
# 区分POST操作和GET操作，在这个接口里面默认POST操作为ajax方式。
# 这里的uid其实是topic id， u是unique的意思，不是user的意思。
